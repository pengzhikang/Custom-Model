...
#ifndef PZK_H_
#define PZK_H
#include "model/pzk-schema_generated.h"
#include "flatbuffers/idl.h"
#include "flatbuffers/util.h"
#include <iostream> // C++ header file for printing
#include <fstream> // C++ header file for file access
#include "model/json11.hpp" // for read json file
using namespace PzkModel;

size_t datalen(DataType a){
    switch(a){
        case(DataType_INT32):
        ...
        case(DataType_UINT8):
        case(DataType_CHAR):
        case(DataType_QSYMMEINT4):
        case(DataType_QSYMMEINT8):
        case(DataType_QASYMMEUINT4):
        case(DataType_QASYMMEUINT8):
            return sizeof(char);
        case(DataType_INT16):
...
public:
    PzkM(const PzkM& c);
    PzkM &operator=(const PzkM &);
    PzkM();
    PzkM(std::string jsonfile);
    ~PzkM();
    void clear_data();
    bool ReadModel(std::string modelfile);
    std::vector<struct TensorsS> get_layer_input(layer_maker l);
    struct TensorsS get_layer_input_by_name(layer_maker l, std::string name);
    std::vector<struct TensorsS> get_layer_output(layer_maker l);
    ...
    uint32_t layer_id = 0;
    // vars for read model
    std::vector<struct TensorsS> rTensors;
    std::vector<layer_maker> rLayers; 
};
...
int layer_maker::get_input_id(std::string input_name){
    int id = -1;
    for(auto i : this->input_id){
...
int layer_maker::get_output_id(std::string output_name){
    int id = -1;
    for(auto i : this->output_id){
...
template <typename DT>
std::vector<DT> layer_maker::get_attr(std::string key){
    std::vector<DT> b;
    if(require_attrs)
    {
        for (size_t i = 0; i < attrs.meta_num; i++)
        {
            ...
            {
                uint32_t datalen = PzkM::datatype_len(attrs.buffer[i].buffer_data);
                uint32_t ele_num = attrs.buffer[i].buffer.size()/datalen;
                DT* dtdata = (DT*)malloc(sizeof(DT));
                uint8_t* dtdata_ptr = (uint8_t*)dtdata;
                for (size_t j = 0; j < ele_num ; j++)
                {
                    if(attrs.buffer[i].buffer.size() > 0){
                        for(size_t k = 0; k < datalen; k++){
                            dtdata_ptr[k] = attrs.buffer[i].buffer[j * datalen + k];
                        }
                        b.push_back(*dtdata);
                    }else{
                        b.push_back((DT)0);
...
PzkM& PzkM::operator=(const PzkM & c){
    meta = c.meta;
    author = c.author;
    version = c.version;
    target_time = &(*c.target_time);
    model_runtime_input_num = c.model_runtime_input_num;
    model_runtime_output_num = c.model_runtime_output_num;
    model_runtime_input_id = c.model_runtime_input_id;
    model_runtime_output_id = c.model_runtime_output_id;
    tensors = c.tensors;
    all_tensor_id = c.all_tensor_id;
    all_layer_id = c.all_layer_id;
    // builder = c.builder;
    tensor_id = c.tensor_id;
    layer_id = c.layer_id;
    rTensors = c.rTensors;
    rLayers = c.rLayers;
    return *this;
...
//set zero to all var
void PzkM::clear_data()
{
    this->author = "";
    this->version = "";
    this->model_name = "";
    this->model_runtime_input_num = 0;
    this->model_runtime_output_num = 0;
    this->model_runtime_input_id.clear();
    this->model_runtime_output_id.clear();
    this->tensors.clear();
    this->all_tensor_id.clear();
    this->all_layer_id.clear();
    this->tensor_id = 0;
    this->layer_id = 0;
}
...
bool PzkM::ReadModel(std::string modelfile)
{
    ...
    // 3.read tensor and layer from my_model
    // 3.1 read base info
...
//get layer's all inputs
std::vector<struct TensorsS> PzkM::get_layer_input(layer_maker l)
{
    std::vector<struct TensorsS> layer_inputs;
    if (l.input_id.size() <= 0 || l.input_id.size() > this->rTensors.size()){
        return layer_inputs;
    }
    for(size_t i = 0; i < this->rTensors.size(); i++){
        if(this->rTensors[i].id == l.input_id[layer_inputs.size()].tensor_id){
            layer_inputs.push_back(this->rTensors[i]);
            if (layer_inputs.size() >= l.input_id.size()){
                break;
            }
        }
    }
...
// get layer's input by name
struct TensorsS PzkM::get_layer_input_by_name(layer_maker l, std::string name){
    uint32_t name_id;
    for(size_t j = 0; j < l.input_id.size(); j++){
        if(l.input_id[j].name == name){
            name_id == l.input_id[j].tensor_id;
            break;
        }
    }
    for(size_t i = 0; i < this->rTensors.size(); i++){
        if(this->rTensors[i].id == name_id){
            return this->rTensors[i];
        }
    }
}

//get layer's all outputs
std::vector<struct TensorsS> PzkM::get_layer_output(layer_maker l)
{
    std::vector<struct TensorsS> layer_outputs;
    if (l.output_id.size() <= 0 || l.output_id.size() > this->rTensors.size()){
        return layer_outputs;
    }
    for(size_t i = 0; i < this->rTensors.size(); i++){
        if(this->rTensors[i].id == l.output_id[layer_outputs.size()].tensor_id){
            layer_outputs.push_back(this->rTensors[i]);
            if (layer_outputs.size() >= l.output_id.size()){
                break;
            }
        }
    }
